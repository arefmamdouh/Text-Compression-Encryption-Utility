...
int compress(map<char, string> charencode, map<string, int>& mapcount) {
	...
	for (int i = 0; i < mapcount.size(); i++) {
		...
		double fre = itr2->second;
		double length = (itr->second).length();
		entropy += ((fre / totalcount) * log2(one / (fre / totalcount)));
		sum += ((fre / totalcount) * length);
		itr2++;
	}
	double compressionratio = sum / 8.0;
	double efficiency = entropy / sum;
	cout << "The compression ratio is " << compressionratio*100 << "% and the efficiency is " << efficiency*100 << "%" << endl;
	ofstream mapofcode;
	...
	ofstream compressed;
	string writepath;
	cout << "Please enter the path of the file to be written: ";
	cin >> writepath;
	compressed.open(writepath);
	while (!compressed) {
		cout << "ERROR!\nPlease enter the path of the file to be written: ";
		cin >> writepath;
		compressed.open(writepath);
	}
	int count = 0;
...
string readCompressedFile(map<string, char>& m) {
...
void decompress(int count) {
	map<string, char> m;
	string readpath = readCompressedFile(m);
	map<string, char>::iterator itr;
	string file;
	ifstream filein;
	filein.open(readpath);
	istreambuf_iterator<char> inputIt(filein), emptyInputIt;
	back_insert_iterator<string> stringInsert(file);
